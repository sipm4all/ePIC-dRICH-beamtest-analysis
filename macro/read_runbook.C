...
//reading the second line for the key
    ...
    while(std::getline(iss_key,field_key, ',')){
        ...
        if (!field_key.empty()) keys.push_back({field_key,"-"});// second value is for default value will be updated later
    }
       ...
       // std::vector<string> line_val;
        ...
        while(std::getline(iss,field,',')){
           ...
           // std::<pair> line_val;
            ...
            if(pos_counter> keys.size()-1) break;
            ...
            //}
            //else{
            if(!field.empty())  keys[pos_counter].second = field;
            if(field.empty())   field = keys[pos_counter-1].second;
            ...
            if(pos_counter == 15 && !field.empty()) first_value = field;//keys[pos_counter].first;
            else if(pos_counter == 16 && !field.empty()) first_value =field;// keys[pos_counter].first;
            else if(pos_counter == 17 && !field.empty()) first_value = field;//keys[17].first;
           // else continue;
